...
def process_dataset(audio_dir, label_csv, output_path):
    """
    Procesa todo el dataset de ENTRENAMIENTO y guarda en formato parquet
    ...
    """
    print("Iniciando procesamiento del dataset de ENTRENAMIENTO...")
    ...
    output_file = Path(output_path) / "audio_features_train.parquet"
    final_df.to_parquet(output_file, index=False, compression='snappy')
    
    print(f"✓ Dataset de ENTRENAMIENTO guardado en: {output_file}")
...
def process_test_dataset(audio_dir, output_path):
    """
    Procesa el dataset de TESTING (sin etiquetas) y guarda en formato parquet
    
    Args:
        audio_dir: Directorio con archivos .wav de testing
        output_path: Ruta donde guardar el archivo .parquet
    """
    print("\n" + "="*50)
    print("Iniciando procesamiento del dataset de TESTING...")
    print("="*50)
    ...
    print(f"Procesando {len(audio_files)} archivos de audio de testing...")
    ...
    # Crear DataFrame con features (sin etiquetas)
    features_df = pd.DataFrame(all_features)
    
    print(f"\nDataset de testing procesado: {len(features_df)} muestras")
    print(f"Número de features extraídas: {len(features_df.columns) - 1}")  # -1 por nombre_archivo
    
    # Guardar en formato parquet
    output_file = Path(output_path) / "audio_features_test.parquet"
    features_df.to_parquet(output_file, index=False, compression='snappy')
    
    print(f"✓ Dataset de TESTING guardado en: {output_file}")
    print(f"✓ Tamaño del archivo: {output_file.stat().st_size / (1024*1024):.2f} MB")
...
if __name__ == "__main__":
    # Configurar rutas
    TRAIN_AUDIO_DIR = r"C:\Users\japal\Documents\TechCapital\Paralinguistic-Speech-Classification-for-Human-Vocalizations-2\data\raw\Training_data"
    TEST_AUDIO_DIR = r"C:\Users\japal\Documents\TechCapital\Paralinguistic-Speech-Classification-for-Human-Vocalizations-2\data\raw\Testing_data"
    ...
    print("="*70)
    df_train = process_dataset(TRAIN_AUDIO_DIR, LABEL_CSV, OUTPUT_DIR)
    
    print(f"\n✓ Entrenamiento completado exitosamente!")
    print(f"\nPrimeras filas del dataset de entrenamiento:")
    print(df_train.head())
    
    # Procesar dataset de TESTING
    print("\n" + "="*70)
    print("PROCESANDO DATASET DE TESTING")
    print("="*70)
    df_test = process_test_dataset(TEST_AUDIO_DIR, OUTPUT_DIR)
    
    if df_test is not None:
        print(f"\n✓ Testing completado exitosamente!")
        print(f"\nPrimeras filas del dataset de testing:")
        print(df_test.head())
    ...
    if df_test is not None:
        print(f"✓ Dataset de testing: {len(df_test)} muestras")
    print(f"✓ Features por muestra: {len(df_train.columns) - 2}")
    print(f"\n✓ Archivos generados:")
    print(f"   - audio_features_train.parquet (con etiquetas)")
    print(f"   - audio_features_test.parquet (sin etiquetas)")
